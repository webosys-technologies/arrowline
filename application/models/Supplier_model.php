<?php  if ( ! defined('BASEPATH')) exit('No direct script access allowed');
    ...
    */
    public function addSupplier($supplier)
    {
        ...
        $sql="INSERT INTO `supplier`(`name`, `email`, `phone`, `street`, `city_id`, `state_id`,`state_code`, `zipcode`, `country_id`,`gstin`,`user_id`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

        if($this->db->query($sql,$supplier))
        {
            return true; 
        }
        else
  ...
  /* ------------------------------------------------------------------*/
    ...
    */
       ...
       */
       ...
       {
          $data=$this->db->query("SELECT s.*,(st.name) as state_name,(c.name) as city_name FROM `supplier` as s 
    ...
    */
    public function deleteSupplier($del)
    {
        $sql="UPDATE supplier set delete_status = ? , delete_date = ? WHERE id = ? ";
   ...
   {
      $sql="SELECT count(id) AS total FROM supplier where status=1 ";
   ...
   {
      $sql="SELECT count(status) AS Active  FROM supplier where delete_status=0 ";
      $query=$this->db->query($sql);
      return $query->row();      
   }
   ...
   {
      $sql="SELECT count(status) AS Deactive  FROM supplier where delete_status=1 ";
      $query=$this->db->query($sql);
      return $query->row();      
   }
