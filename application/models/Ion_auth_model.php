...
class Ion_auth_model extends CI_Model
{
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$group1['user_id']=$id;
		$group1['group_id']=$additional_data['group_id'];
		$this->add_group($group1);
	...
	**/
    ...
    {
		$this->db->select('date,sum(total_amount) as amount');
		$this->db->where('date BETWEEN DATE_SUB(NOW(), INTERVAL 30 DAY) AND NOW()');
    ...
    {	$this->db->select('account_type,bank_name,opening_balance');
    	$this->db->limit(5);
    	$this->db->order_by('id','desc');
    ...
    {	$this->db->select('date,description,amount');
    	$this->db->limit(5);
    	$this->db->order_by('id','desc');
    ...
    {     
    	$this->db->select('SUM(sales_amount) as amount,
                        SUM(paid_amount) as pamount');
    ...
    {
    	$this->db->select('SUM(e.amount) as expense,ec.name');
    	$this->db->from('expense e');
    	$this->db->join('expense_category ec','e.category_id = ec.id');
    	$this->db->where('date BETWEEN DATE_SUB(NOW(), INTERVAL 30 DAY) AND NOW()');
    ...
    {
    	$this->db->select('SUM(sales_amount) as total,SUM(paid_amount) as paid');
