...
<body>
	<table style="border:0px solid black;margin: 0px; width:100%; font-size: 14px;height:100%; font-family: arial;">
		<tr>
			<td style="font-size: 16px;font-weight: bold;"><b><?php if(isset($country->name)){echo $country->name;}?></b></td>
		...
		<tr>
			<td><?php if(isset($country->city_name)){echo $country->city_name;}?>,<?php if(isset($country->state_name)){echo $country->state_name;} ?></td>
			<td align="right">INVOICE #<?php if(isset($value->reference_no)){echo $value->reference_no;}?></td>
		</tr>
		<tr>
			<td>Zip Code :<?php if(isset($country->zip_code)){echo $country->zip_code;}?></td>
	...
	<table style="border:0px solid black;margin: 0px; width:100%; font-size: 14px;height:100%; font-family: arial;">
		<tr>
			...
			<td style="font-size: 20px;font-weight: bold; padding-left:50px;"><b><?php echo $this->lang->line('lbl_bill_to');?> </b></td>
			<td style="font-size: 16px;font-weight: bold;padding-left:30px;"><b><?php echo $this->lang->line('lbl_shipping_details');?> </b></td>
		</tr>
		<tr>
		...
		<tr>
			<td><?php if(isset($country->city_name)){echo $country->city_name.','.$country->country_name;}?></td>
			<td style="padding-left:50px;"><?php if(isset($value->cust_city)){echo $value->cust_city.','.$value->country_name; }?></td>
			<td style="padding-left:30px;"><?php if(isset($address->state_name)){echo $address->state_name; }?></td>
		</tr>
		<tr>
			<td><?php if(isset($country->zip_code)){echo "Zip code : ".$country->zip_code;}?></td>
			<td style="padding-left:50px;"> <?php if(isset($value->cust_zipcode)){echo "Zip code :".$value->cust_zipcode;} ?></td>
			<td style="padding-left:30px;"><?php if(isset($address->country_name)){echo $address->country_name; }?> - <?php if(isset($address->zip_code)){echo $address->zip_code; }?></td>
		</tr>
		<tr>
	...
	<table width="100%" border="1" cellspacing="0" style="border: 1px solid black; border-collapse: collapse" class="table">
		...
		<tbody>
			...
			<tr>
				...
				<td align="right">
					<?php if(isset($value->rate)){echo number_format((float)$value->rate, 2, '.', '');}?>		
				</td>

				<td align="right">
					<?php if(isset($total_sales)){echo number_format((float)$total_sales, 2, '.', '');}?>		
				</td>
				<td align="right"><?php if(isset($value->discount)){echo $value->discount;}?></td>
				<td align="right">
					<?php if(isset($discount_value)){echo number_format((float)$discount_value, 2, '.', '');}?>		
				</td>
				<td align="right">
					<?php if(isset($taxable_value)){echo number_format((float)$taxable_value, 2, '.', '');}?>		
				</td>
				...
				<td align="right">
					<?php if(isset($value->amount)){echo number_format((float)$value->amount, 2, '.', '');}?>	
				</td>
			</tr>
			<?php 
			$cnt++;
			...
			<tr>
	              <td colspan="13" align="right" class="footerpad">
	              Shipping Charges
